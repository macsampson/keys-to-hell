...
export class Player extends GameObject implements IPlayer {
    ...
    public takeDamage(damage: number): boolean {
        ...
        if (this.currentShield > 0) {
            ...
            console.log(`Shield absorbed ${shieldAbsorbed} damage, ${this.currentShield} shield remaining`);
        }

        // Check for rewind ability before taking fatal damage
        if (actualDamage >= this.health && this.hasRewind && this.rewindCharges > 0) {
            this.activateRewind();
            return false; // Prevent death
        }

        // Apply remaining damage to health
        const isDead = actualDamage > 0 ? super.takeDamage(actualDamage) : false;
    ...
    private activateRewind(): void {
        ...
        console.log(`Rewind activated! Healed ${this.rewindHealAmount} HP. Charges remaining: ${this.rewindCharges}`);
        
        // Visual effect for rewind
        const scene = this.scene as any;
        if (scene.add) {
            const rewindText = scene.add.text(this.x, this.y - 50, 'REWIND!', {
                fontSize: '20px',
                color: '#00ff00',
    ...
    private activateTimeDilation(): void {
        ...
        console.log(`Time dilation activated: ${this.dilationStrength * 100}% slowdown for ${this.dilationDuration}ms`);
    ...
    private activateStasisField(): void {
        ...
        console.log(`Stasis field activated, froze ${affectedEnemies.length} enemies for ${this.stasisDuration}ms`);
        ...
        scene.time.delayedCall(this.stasisDuration, () => {
            ...
            console.log(`Stasis field ended, restored movement to ${affectedEnemies.length} enemies`);
        });
    }
    ...
    public onWordCompleted(): void {
        ...
        if (this.hasTypingShield && this.shieldPerWord > 0) {
            this.currentShield = Math.min(this.currentShield + this.shieldPerWord, this.maxShield);
            console.log(`Generated ${this.shieldPerWord} shield from typing. Current shield: ${this.currentShield}/${this.maxShield}`);
        }
    }
    ...
    public onPerfectWord(): void {
        ...
        if (this.hasWordBarrier && this.barrierStrength > 0) {
            this.currentShield = Math.min(this.currentShield + this.barrierStrength, this.maxShield);
            console.log(`Generated ${this.barrierStrength} barrier from perfect word. Current shield: ${this.currentShield}/${this.maxShield}`);
        }
    }
