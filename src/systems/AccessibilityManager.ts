...
export class AccessibilityManager {
  ...
  private setupCanvasAccessibility(): void {
    ...
    canvas.setAttribute('aria-label', 'Typing Hell - A typing-based action game')
  ...
  public announce(message: string, priority: 'polite' | 'assertive' = 'polite'): void {
    ...
    const element = priority === 'assertive' ? this.statusElement : this.ariaLiveRegion
    if (element) {
  ...
  private handleResize(): void {
    ...
    this.scene.events.emit('accessibilityResize', {
      ...
      gameWidth,
      gameHeight,
      scale: Math.min(window.innerWidth / gameWidth, window.innerHeight / gameHeight)
    })
  }
  ...
  private saveOptions(): void {
    try {
      localStorage.setItem('typing_hell_accessibility', JSON.stringify(this.options))
    } catch (error) {
